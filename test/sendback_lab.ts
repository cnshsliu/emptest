...
describe("Test: ", { timeout: 5000 }, () => {
  ...
  it("sendback a work in parallels", async () => {
    ...
    let errThrown = false;
    await SDK.sleep(500);
    let ret = await SDK.sendback(getEid(0), wfid, action22_todoid);
  ...
  it("do task action21/22", { timeout: 10000 }, async () => {
    await SDK.sleep(200);
    await SDK.doWorkByNode(getEid(0), wfid, "action21");
    await SDK.sleep(200);
    await SDK.doWorkByNode(getEid(0), wfid, "action22");
    await SDK.sleep(500);
    let wlist = await SDK.getWorklist(getEid(0), { wfid: wfid, status: "ST_RUN" });
  ...
  it("Sendback from action3", { timeout: 5000 }, async () => {
    await SDK.sendback(getEid(0), wfid, action3_todoid);
  ...
  it("do task to action3", { timeout: 10000 }, async () => {
    ...
    await SDK.doWorkByNode(getEid(0), wfid, "action22");
    await SDK.sleep(200);
    await SDK.doWorkByNode(getEid(0), wfid, "action3");
    await SDK.sleep(500);
    let wlist = await SDK.getWorklist(getEid(0), { wfid: wfid, status: "ST_RUN" });
    expect(wlist.total).to.equal(2);
    action41_todoid =
  ...
  it("Sendback from action41", async () => {
    try {
      let ret = await SDK.sendback(getEid(0), wfid, action41_todoid);
  ...
  it("do task action41/42", { timeout: 10000 }, async () => {
    await SDK.sleep(2000);
    action41_todoid = await SDK.doWorkByNode(getEid(0), wfid, "action41");
    await SDK.sleep(2000);
    let wlist = await SDK.getWorklist(getEid(0), { wfid: wfid, status: "ST_RUN" });
    expect(wlist.total).to.equal(1);
  ...
  it("Sendback from action5", { timeout: 5000 }, async () => {
    let ret = await SDK.sendback(getEid(0), wfid, action5_todoid);
  ...
  it("do task action41/42/5", { timeout: 10000 }, async () => {
    ...
    try {
      ...
      await SDK.doWorkByNode(getEid(0), wfid, "action42");
      await SDK.sleep(200);
    } catch (err) {
