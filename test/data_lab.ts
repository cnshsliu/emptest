...
describe('Test and_or logic', { timeout: 5000 }, () => {
	...
	it('Step 2: Upload template', async () => {
		...
		expect(ret.tplid).to.equal('test_data');
	...
	it('Step 3: Start Workflow', async () => {
		...
		const ret = await SDK.startWorkflow('test_data', wfid, '', pbo);
	...
	it('Step 5: Do action1', { timeout: 5000 }, async () => {
		...
		let theTodo = wlist.objs[0];
		let action1_todoid = theTodo.todoid;
		let ret = await SDK.post('/data/set', {
			context: {
				tenant: theTodo.tenant,
				eid: theTodo.eid,
				tplid: theTodo.tplid,
				wfid: theTodo.wfid,
			},
			key: 'key1',
			data: { name: 'lucas', score: 80 },
		});
		//根据tplid差，会多于一个，因为同一个tplid下会有多个wfid
		ret = await SDK.post('/data/get', {
			query: { 'context.tplid': theTodo.tplid, key: 'key1' },
		});
		expect(ret.length > 0).to.equal(true);
		//根据wfid查，只会有一个
		ret = await SDK.post('/data/get', {
			query: { 'context.wfid': theTodo.wfid, key: 'key1' },
		});
		expect(ret[0].data.score).to.equal(80);
	});

	it('cleaning up', async () => {
		await SDK.sleep(3000);
		await SDK.destroyWorkflowByTplid('test_data');
		await SDK.deleteTemplateByTplid('test_data');
		await SDK.login(SITE_ADMIN.account, SITE_ADMIN.password);
		for (let i = 0; i < testUsers.length; i++) {
			await SDK.removeUser(testUsers[i].account, SITE_PWD);
		}
