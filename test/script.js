...
describe("Test: ", () => {
  ...
  it("Upload template", async () => {
    let ret = await SDK.putTemplate(fs.readFileSync(TEST_TEMPLATE_DIR + "/script1.xml", "utf8"));
    expect(ret.tplid).to.equal("script1");
  });

  it("START Workflow script1", async () => {
    let ret = await SDK.startWorkflow("script1", wfid);
  });
