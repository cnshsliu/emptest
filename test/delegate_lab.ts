...
describe('Test Delegation: ', { timeout: 5000 }, () => {
	...
	it('test delegation', async () => {
		...
		res = await SDK.post('delegation/new', {
			delegatee: getEid(2), //delegate to user 2
			begindate: begindate, //today
			enddate: enddate, // +3 days
		});
		...
		//// user    1     revoke delegation
		res = await SDK.login(testUsers[1].account, testUsers[1].passwd);
		res = await SDK.post('delegation/from/me');
		let ids = res.map((x) => x._id);
		res = await SDK.post('delegation/revoke', { ids });
